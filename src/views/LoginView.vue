...
<template>
  <main class="h-screen w-screen flex items-center justify-center p-6 bg-[var(--dash-bg)] transition-colors duration-700">
    <div class="max-w-md w-full glass-tile p-12 text-center flex flex-col items-center">
      <div class="mb-8">
        <h1 class="text-5xl font-black tracking-tighter mb-2 text-[var(--dash-text)]">{{ $t('appTitle') }}</h1>
        <p class="text-[var(--dash-text-muted)] uppercase tracking-[0.2em] text-xs font-bold">{{ $t('appSubtitle') }}</p>
      </div>

      <p class="text-[var(--dash-text-muted)] mb-10 leading-relaxed">
        {{ $t('loginDescription') }}<br>
        {{ $t('loginPrompt') }}
      </p>
      ...
      >
        ...
        <span>{{ isLoggingIn ? $t('connecting') : $t('signInWithGoogle') }}</span>
      </button>

      <div class="mt-12 pt-8 border-t border-[var(--dash-tile-border)] w-full">
        <div class="text-[10px] text-[var(--dash-text-muted)] uppercase tracking-widest font-bold">{{ $t('appVersion') }}</div>
      </div>
    </div>
  </main>
</template>
